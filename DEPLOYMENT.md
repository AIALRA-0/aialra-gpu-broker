...
首轮切换需要先让 H3、Live、Manga 的 GPU 取队列暂停，确认它们和后端都已停止并卸载，验证各 GPU 入口在门禁关闭时拒绝新提交，然后启动 Owner、观察到有直接事实支持的 `FREE`，再依次恢复项目队列。真实 H3 视频、Live 录音后 GPU 阶段、Manga 单页及三组交接和故障恢复全部成功后，才能把 Owner 服务启动类型设为自动。切换时不导入旧 Broker 的 session、job 或 permit；公网隧道与 Authentik 仍只连接仪表盘端口 18765
