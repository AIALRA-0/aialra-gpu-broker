...
已观察到的故障是旧 Broker 进程退出、端口短暂没有监听；没有观察到 Owner 端口被其他进程占用，也没有证据表明本机存在恶意项目进程。Owner 仍未投入生产

本机回环绑定阻止公网直接访问，却不能证明回应请求的进程就是 Coordinator。如果服务退出后其他进程占用相同端口，直接发送 Bearer 令牌会把可复用凭据交给错误进程；只看响应 JSON 也可能把伪造的 `ACQUIRED` 当作许可。文件 ACL 只限制谁能读本地文件，不能证明 HTTP 响应来自正确进程

因此 v1 保留现有请求与响应签名：请求不发送明文令牌，客户端只接受与本次挑战匹配的签名 `ACQUIRED`。签名失败只使新工作等待，不取消已提交模型计算。这一层防护针对错误监听者和不同账户的本机进程，不宣称能够阻止已经能读取同一项目令牌的恶意同账户进程或 Windows 管理员

本轮不增加传输证书、第二套令牌、每阶段许可或新的业务签名。如果未来改为受操作系统身份约束且能认证服务端的本机通道，再比较能否删除本协议；单凭“只绑定回环地址”不能直接删除响应认证
