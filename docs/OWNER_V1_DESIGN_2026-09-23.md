...
| `last_observed_at` | 当前状态所需来源的最近一次可信观察时刻；分来源时间仍单独记录，局部采样不伪装成三项目都已重新观察。此字段**不是超时后杀任务或自动释放的租约** |
...
v1 暂以 10 秒作为本次操作所需整卡遥测与项目观察的最大证据年龄；超过即不用于 acquire／release，Owner 进入或保持 `UNKNOWN`，在跑任务不取消。整卡采样由 Coordinator 自己生成时间戳；项目观察须由 Coordinator 向预配置回环地址发起带随机挑战的新请求，项目适配器现查后端并用本项目凭据对响应签名，回传挑战和采样时刻。Coordinator 不把项目凭据发给观察端口，避免端口被其他本机进程占用时泄漏；核对签名、挑战、同机时钟偏差、请求耗时，并以自己收到响应的时刻计算缓存年龄。超时、复用旧响应或无法确定采样时间均视为 `UNKNOWN`。本次所需来源与整卡采样必须在一次序列化的准入判定中核对时间戳与当前 Owner 行；乱序或较旧响应不得覆盖较新的观察。这个阈值是证据有效期，不是运行任务的死亡计时器；局部观察不能刷新未采样项目的时间戳

**正常与恢复分开：** 首次启动、Coordinator 重启、状态不明或相关证据过期时，必须取得 H3、Live、Manga 和整卡的四路新鲜直接事实，才可恢复 `FREE` 或匹配的 `OWNED`。稳定 `FREE` 下 acquire 只请求申请项目的 `IDLE`／入口已封闭事实及整卡安全空闲；稳定 `OWNED` 下轮询、等待者查询和 release 只请求当前 Owner 与整卡。稳定 `FREE` 的定期观察只读整卡。非 Owner 项目观察端停机不会因此冻结已有正常交接；申请项目、当前 Owner 或 GPU 的必要观察失败仍转 `UNKNOWN`。这项简化以首次切换前**三项目每个实际 GPU 入口均已验证安装并启用门禁**为前提；整卡数据在当前 Owner 忙时不能归因其他项目的偷偷使用，未覆盖入口不得参与生产切换
...
H3 普通视频的 Owner 窗口应止于最后一个 4080 后端工作确定终态、入口封闭且模型安全卸载。下载、校验与落盘若只使用 CPU／网络，可在 release 后继续；业务 task 必须等本地成品和 take 真正保存成功才标为 `completed`。这两个完成时刻分别属于 GPU 所有权与 H3 业务，不能合并成一个状态
