...
`IDLE` 必须同时满足：本窗口入口已封闭，旧 `owner_instance` 不会再发起下一 GPU 阶段；后端没有未完成调用；本项目模型已释放；相关子进程已退出或经监督进程确认不会使用 4080。H3 视频两个阶段之间即使显存暂低，也要报告 `BUSY`；Live 队列中尚未领取的任务可以等待下一窗口，但当前窗口不能在释放后继续领任务；Manga 导入线程和生成队列须共用同一入口封闭判断。
